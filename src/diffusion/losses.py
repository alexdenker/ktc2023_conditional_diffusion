...
def score_based_loss_fn(x, model, sde, cond_inp, eps=1e-5):
    ...
    perturbed_x_cat = torch.cat([perturbed_x, cond_inp], dim=1) # add cond_inp as extra channels
    score = model(perturbed_x_cat, random_t)
    loss = torch.mean(torch.sum((score * std[:, None, None, None] + z)**2, dim=(1,2,3)))
    
    return loss

def epsilon_based_loss_fn(x, model, sde, cond_inp):
    ...
    perturbed_x_cat = torch.cat([perturbed_x, cond_inp], dim=1) # add cond_inp as extra channels
    zhat = model(perturbed_x_cat, random_t)
